...
function loadHttpSchema(schemaUrl) {
  return new Promise((resolve, reject) => {
    ...
    const requestOptions = url.hostname === 'localhost' ? { family: 4 } : undefined;
    const request = client.get(url, requestOptions, (response) => {
      ...
      response.on('end', () => {
        ...
        if (!response.statusCode || response.statusCode < 200 || response.statusCode >= 300) {
          reject(new Error(`Failed to fetch OpenAPI schema from ${schemaUrl}: ${response.statusCode} ${response.statusMessage}`));
          return;
        }
        try {
          resolve(JSON.parse(body));
        } catch (error) {
          reject(new Error(`OpenAPI schema from ${schemaUrl} is not valid JSON: ${error.message}`));
        }
      });
    });
    request.on('error', (error) => reject(new Error(`Failed to fetch OpenAPI schema from ${schemaUrl}: ${error.message}`)));
  });
}
...
async function main() {
  ...
  const outputPath = process.env.OPENAPI_CODEGEN_SCHEMA_PATH || DEFAULT_OUTPUT_PATH;
  const allauthSchemaUrl = process.env.ALLAUTH_OPENAPI_SCHEMA_URL || DEFAULT_ALLAUTH_SCHEMA_URL;
  const allauthOutputPath = process.env.ALLAUTH_OPENAPI_CODEGEN_SCHEMA_PATH || DEFAULT_ALLAUTH_OUTPUT_PATH;
  const schema = await loadSchema(schemaUrl);
  const allauthSchema = await loadSchema(allauthSchemaUrl);
  const transformed = transformOpenApiTags(schema);
  const transformedAllauth = transformAllauthSchema(allauthSchema);

  await writeSchema(outputPath, transformed);
  await writeSchema(allauthOutputPath, transformedAllauth);
