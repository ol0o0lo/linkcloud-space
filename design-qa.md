...
- 参考图：`/var/folders/l5/hrrcn7g1065bt7_hgmsywwb80000gn/T/codex-clipboard-6404e8cd-c881-4261-869f-a66c0d7f0870.png`
- 实现截图：`/var/folders/l5/hrrcn7g1065bt7_hgmsywwb80000gn/T/codex-house-list-range-implementation.png`
- 参考图尺寸：2946 × 1536 px；按截图文字密度推断为约 1473 × 768 CSS px、DPR 2。
- 实现视口：1280 × 720 CSS px、DPR 2；实现截图为 2560 × 1440 px。
- 状态：已选择“无楼栋治理样板”项目，列表为空；随后验证“清除项目筛选”。

实现视口略小于参考图，因此重点比较范围导航、表格容器和筛选提示区域，而非把全页边缘做像素级对齐。

## 对比结论

### 全局布局

- 左侧房源范围与右侧列表卡片高度一致，实测均为 540 px。
- 房源范围的搜索区和快捷操作区固定，中间项目/楼栋区为 `overflow-y: auto`，数据较多时不会撑高页面。
- 当前项目/楼栋不再通过独立卡片占用一行，而是出现在表格工具栏的左侧；关键词、房态和新建操作仍保留在右侧。

### 视觉与可用性核对

- 字体与层级：沿用现有 Ant Design 字体、权重和列表工具栏层级；范围名称使用正文强调，清除为轻量链接操作。
- 间距与布局节奏：两列保持 16 px 间距，容器圆角、边框和内边距沿用主题 token；左右高度对齐。
- 颜色与 token：范围提示使用 `colorFillQuaternary`，选中和可清除状态继续采用主题语义色。
- 图标与资源：使用已有项目/楼栋图标，不新增图像资源或自定义图形。
- 文案与交互：选择项目后可见项目名和“清除”；清除后范围提示消失，列表回到全部房源。

## 比较历史

1. 初次实现中，较窄桌面宽度下“当前项目”文字会压缩项目名称。
2. 已移除可见的“当前项目/当前楼栋”前缀，保留图标、完整实体名称与“清除”，无障碍名称仍包含完整筛选上下文。
3. 修正后，项目名在工具栏内完整显示；浏览器控制台无 error。

## 发现

- 无可操作的 P0、P1 或 P2 差异。
- P3：极窄桌面宽度下，工具栏会优先保留筛选范围和主要搜索操作，其余表格列按现有横向滚动策略查看；这是宽表的既有交互，不影响本次范围导航改造。
