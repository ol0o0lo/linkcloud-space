...
from apps.media.services import RESOURCE_TYPE_RULES, get_oss_token, register_media_file, upload_and_register

router = Router(tags=["媒体/文件"])


def _required_scope_for_resource_type(resource_type: str) -> str | None:
    rule = RESOURCE_TYPE_RULES.get(resource_type)
    if rule is None or len(rule["scopes"]) != 1:
        return None
    return next(iter(rule["scopes"]))
...
def oss_token(request, params: OssTokenIn = Query(..., description="上传凭证请求参数。")):
    ...
    if params.resource_type is not None and params.resource_type not in ResourceType.values:
        raise HttpError(422, f"无效的 resource_type: {params.resource_type}")
    ...
    result = get_oss_token(scope=params.scope, object_id=object_id, filename=params.filename, resource_type=params.resource_type)
...
def confirm_upload(request, payload: MediaFileConfirmIn):
    ...
    scope = _required_scope_for_resource_type(payload.resource_type)
    object_id = None
