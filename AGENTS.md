...
- **Static / media**: WhiteNoise serves Vite-hashed assets in production with `Cache-Control: max-age=31536000, immutable` (regex defined in settings). Media uploads use `apps/base/storage.py:S3MediaStorage` which handles the Docker-internal vs. browser endpoint URL split for MinIO. 业务模型通常以 `JSON list[int]` 保存 `media_ids`，通过 `apps.media.services.validate_media_ids()` / `get_media_list_info()` 接入媒体平台，并通过 `settings.MEDIA_REFERENCE_PROVIDERS` 参与延迟清理。
