...
- 当前不设计历史跨天 UV 聚合，也不增加访客事实表。
- `trends` 中按天计算的 `unique_visitors` 保持有效。
- 目前数据全部来自明细表，区间 UV 仍可实时计算。
- 后续启用 30 天冷热分层后，包含历史聚合数据的区间不提供整体或目标跨天 UV，不能把每日 UV 相加冒充区间 UV。
- 该限制只影响历史聚合查询，最近 30 天的明细查询仍可计算精确区间 UV。
...
7. 当前没有事件自动清理任务；删除目标也不会删除历史事件，查询时会回退为类型和 ID 标签。
