...
function getMapReturnTo() {
  ...
  return value === '/dashboard/property-rental/map' ||
    value?.startsWith('/dashboard/property-rental/map?')
    ? value
    : undefined;
}
      ...
      render: (_value, record) => {
        ...
        return (
          ...
          >
            ...
            />
            <Link to={`/property-rental/estates/${record.id}`}>
  ...
  const buildingColumns: ProColumns<BuildingOut>[] = [
    ...
    { title: '地址', dataIndex: 'address', render: (value) => value || '-' },
    {
      ...
      render: (_value, record) => (
        <ResponsiveActions>
          <a
            href={`/dashboard/property-rental/houses/new?building_id=${record.id}`}
          >
            登记房源
          </a>
          <Button
      ...
      >
        ...
        {editingEstate && estateBuildings.data?.items.length ? (
          <Card
            ...
            extra={
              <a
                href={`/dashboard/property-rental/estates?view=buildings&estate_id=${editingEstate.id}`}
              >
                查看全部楼栋
              </a>
            }
