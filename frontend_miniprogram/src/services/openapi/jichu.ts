...
export function teamsTeamIdMembersUserIdUsingPost({
  params,
  options,
}: {
  // 叠加生成的Param类型 (非body参数openapi默认没有生成对象)
  params: API.TeamsTeamIdMembersUserIdUsingPostParams;
  options?: CustomRequestOptions_;
}) {
...
export function teamsTeamIdMembersUserIdUsingDelete({
  params,
  options,
}: {
  // 叠加生成的Param类型 (非body参数openapi默认没有生成对象)
  params: API.TeamsTeamIdMembersUserIdUsingDeleteParams;
  options?: CustomRequestOptions_;
}) {
  const { team_id: param0, user_id: param1, ...queryParams } = params;
