...
describe('HouseLocationPoiCard', () => {
  ...
  it('shows a maintenance action when the building has no coordinates', () => {
    render(
      <HouseLocationPoiCard
        ...
        lat={null}
        lng={null}
        returnTo="/dashboard/rental/properties/10"
      />,
    );
    ...
    expect(screen.getByRole('link', { name: '去维护定位' })).toHaveAttribute(
      'href',
      '/dashboard/rental/properties/estates?view=buildings&task=building_location&building_edit=2&return_to=%2Fdashboard%2Frental%2Fproperties%2F10',
    );
  ...
  it('renders the location map and nearby POIs by category', async () => {
    ...
    expect(
      ...
      'href',
      '/dashboard/rental/properties/map?selected_building_id=2&center_lat=23.137313&center_lng=113.343920&zoom=16',
    );
