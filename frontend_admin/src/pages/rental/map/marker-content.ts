...
export function createBuildingInfoWindowContent(
  ...
  appendLink(
    actions,
    '查看房源',
    `${options.adminBasePath}/rental/properties/list?building_id=${building.id}`,
    true,
  );
  appendLink(
    actions,
    '楼栋详情',
    `${options.adminBasePath}/rental/properties/buildings/${building.id}?return_to=${encodeURIComponent(options.returnTo)}`,
  );
  appendLink(
    actions,
    '编辑位置',
