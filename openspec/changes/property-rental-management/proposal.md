...
- 收紧关键数据约束：联系人归属、租约并发、房源状态重算、房源媒体引用仅允许合法 `MediaFile` 且首图为封面
- 复用现有 `apps/media` + `S3MediaStorage` / MinIO 文件体系存储房源图片/视频和租约合同，并按 `docs/media-platform.md` 使用 `media_id` 平铺引用对象
- 新增 Django admin 注册
...
- 接入 `MEDIA_REFERENCE_PROVIDERS`，为房源域媒体引用提供延迟清理依据
