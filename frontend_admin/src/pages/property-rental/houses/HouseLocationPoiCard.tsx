...
export default function HouseLocationPoiCard({
  ...
  const mapHref = coordinates
    ? `/dashboard/property-rental/map?selected_building_id=${buildingId}&center_lat=${coordinates.lat.toFixed(6)}&center_lng=${coordinates.lng.toFixed(6)}&zoom=16`
    : undefined;
  const editHref = `/dashboard/property-rental/estates?view=buildings&task=building_location&building_edit=${buildingId}&return_to=${encodeURIComponent(returnTo)}`;
