...
describe('House new page', () => {
  beforeEach(() => {
    ...
    mockListEstates.mockResolvedValue({
      items: [{ id: 1, name: 'xinghewan', display_name: '星河湾花园' }],
      total: 1,
      page: 1,
      page_size: 20,
    });
  ...
  it('walks through a four-step wizard and creates a house draft without forcing a landlord', async () => {
    ...
    fireEvent.change(screen.getByLabelText('挂牌租金'), {
      target: { value: '4200' },
    });
    await clickNextWhenEnabled();
    ...
    expect(mockCreateHouse.mock.calls.at(-1)?.[0]).not.toHaveProperty(
      'landlord_id',
    );
  ...
  it('shows carried-in source context when entering from building and landlord flows', async () => {
    window.history.pushState(
      {},
      '',
      '/rental/properties/new?building_id=10&landlord_id=20',
    );
  ...
  it('uses the selected building when setting the default building', async () => {
    ...
    await waitFor(() =>
      expect(mockSetDefaultBuilding).toHaveBeenCalledWith(10),
    );
  });
  ...
  it('shows landlord phone in the landlord selector', async () => {
    ...
    expect(
      (await screen.findAllByText('张房东 / 13800000000')).at(-1),
    ).toBeDefined();
  });

  it('searches remote building options with pagination parameters', async () => {
    render(
      <QueryClientProvider client={new QueryClient()}>
        <HouseNewPage />
      </QueryClientProvider>,
    ...
    await screen.findAllByText('星河湾 / 1 栋');
    const buildingSelect = screen.getByLabelText('楼栋');
    fireEvent.mouseDown(buildingSelect);
    fireEvent.change(buildingSelect, { target: { value: '星河' } });
  ...
  it('prefills landlord from query param', async () => {
    ...
    expect(screen.getAllByText('张房东 / 13800000000').length).toBeGreaterThan(
      0,
    );
  });
  ...
  it('prefills building from query param', async () => {
    ...
    expect(
      (await screen.findAllByText('星河湾 / 2 栋')).length,
    ).toBeGreaterThan(0);
    fireEvent.change(screen.getByLabelText('房号'), {
      target: { value: '2801' },
    });
  ...
  it('treats warning-only media gaps as publishable under organization publish rules', async () => {
    ...
    expect(
      screen.queryByText('保存后可直接进入发布流程'),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText('提醒项：缺封面、图片不足、缺户型图'),
    ).not.toBeInTheDocument();
  ...
  it('separates the core layout from kitchen and balcony in the final summary', async () => {
    render(
      <QueryClientProvider client={new QueryClient()}>
        <HouseNewPage />
      </QueryClientProvider>,
    ...
    await clickNextWhenEnabled();
    await screen.findByText('补充挂牌与户型');
    fireEvent.change(screen.getByLabelText('卧室'), { target: { value: '2' } });
    fireEvent.change(screen.getByLabelText('客厅'), { target: { value: '1' } });
    fireEvent.change(screen.getByLabelText('卫生间'), {
      target: { value: '1' },
    });
    fireEvent.change(screen.getByLabelText('厨房'), { target: { value: '1' } });
    fireEvent.change(screen.getByLabelText('阳台'), { target: { value: '2' } });
    await clickNextWhenEnabled();
    await screen.findByText('上传图片与视频');
    await clickNextWhenEnabled();

    expect(await screen.findByText('确认房源资料')).toBeInTheDocument();
    expect(screen.getByText('2房1厅1卫')).toBeInTheDocument();
  ...
  it('allows saving a house as soon as the minimum fields are complete', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('button', { name: '保存房源' })).toBeEnabled(),
    );
  ...
  it('creates a landlord from the house form and selects it', async () => {
    ...
    expect(
      (await screen.findAllByText('李房东 / 13900000000')).length,
    ).toBeGreaterThan(0);
  ...
  it('creates a building in a dialog from the first step of the wizard', async () => {
    ...
    fireEvent.click(
      (await screen.findAllByText('星河湾花园')).at(-1) as HTMLElement,
    );
  ...
  it('requires an address when creating a standalone building', async () => {
    ...
    expect(
      await screen.findByText('非小区楼栋必须填写楼栋地址'),
    ).toBeInTheDocument();
