...
function anonymousId() {
  const saved = platformStorage.persistent.getItem(ANALYTICS_ANONYMOUS_ID_KEY)
  if (saved)
    return saved
  const value = createId()
  platformStorage.persistent.setItem(ANALYTICS_ANONYMOUS_ID_KEY, value)
  return value
}
...
export function flushAnalyticsEvents() {
  ...
  flushing = httpPost<AnalyticsCollectResult>(
    ...
    undefined,
    undefined,
    { hideErrorToast: true, requestScope: { kind: 'public' } },
  )
    .catch(() => {
