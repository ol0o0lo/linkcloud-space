...
- 用户已有保存顺序时，前端新增列仍按当前前端默认位置插入，并保留已有列的用户相对顺序。
- 表头设置读取发生非 404 错误时，页面继续使用前端默认状态，但本次页面会话不再向远端写入表头设置。
- 同一列表的写入在前端串行执行，只保留最新待写入状态，避免较早请求覆盖较新的用户操作。
- 第一期以房源列表作为试用入口。
...
后续新增同类内部偏好时统一使用 `internal.*` key，即可自动从设置页面隐藏。个人设置页面直接消费后端过滤后的列表，不需要 `is_user_editable` 一类字段，也不需要根据额外元数据再次判断是否展示。需要特殊写入约束的内部设置，再由对应专用接口保护。
...
1. 从当前 `columns` 递归提取稳定列 key 和前端默认顺序。
2. 根据列定义和可选的 `defaultValue` 生成运行时默认状态，包括默认显示、默认顺序以及前端定义的 `fixed` 和 `disable`；没有显式默认值的普通列按 `show: true` 处理。默认顺序优先使用 `defaultValue.order`，未显式提供时使用当前 `columns` 的展开顺序。
3. 过滤服务端配置中当前列表不存在的列 key。
4. 过滤用户不可持久化的字段并校验字段类型。
5. 合并显示、固定位置等状态，用户配置覆盖前端默认状态，`disable` 始终由前端默认状态覆盖用户数据。
6. 按下述顺序规则合并前端新增列与用户已保存顺序，并为全部当前列重新生成连续的运行时 `order`。
7. 将合并结果作为可直接传给 `columnsState.value` 的值返回。
...
1. 以当前前端默认列顺序建立全部位置槽位。
2. 存在有效用户 `order` 的旧列按用户顺序排列。
3. 没有用户 `order` 的列保留其当前前端默认槽位；其中包括新增加的列。
4. 将按用户顺序排列的旧列依次填入剩余槽位。
5. 最终为全部当前列重新编号为连续的 `order`，避免默认顺序和用户顺序出现重复值。

例如，旧列为 `A、B`，用户调整为 `B、A`；前端之后把新列 `C` 的默认位置定义在二者中间，当前默认顺序为 `A、C、B`，合并后的用户顺序为 `B、C、A`。这样新列位于当前前端定义的位置，同时不会重置旧列之间的用户相对顺序。
...
- 同一钩子的 PUT 和 DELETE 串行执行；请求进行期间发生的新变化只覆盖待执行任务，不并发发送多份同一列表配置。
- 当前请求完成后只执行最新待写入任务，较早响应不得覆盖更新后的本地状态或查询缓存。
...
6. 防抖结束后，钩子将当前列表的完整持久化列状态加入串行写入队列；队列只保留最新待执行任务。
...
- 通用单项 GET 返回非 404 错误：不阻塞业务列表，使用前端默认状态，并提示“表头设置加载失败，当前调整不会保存”。由于前端不知道服务端原配置，本次页面会话锁定为只读持久化状态；`onChange` 仍更新本地表格，但不发送 PUT、DELETE，也不在卸载时补交请求。用户刷新页面并成功读取设置后恢复保存能力。
...
- 同一列表的多次写入串行执行，进行中的请求结束后只发送最新待写入状态。
- 与默认列状态等价时执行 DELETE。
- GET 404 和非 404 失败时都回退到默认列。
