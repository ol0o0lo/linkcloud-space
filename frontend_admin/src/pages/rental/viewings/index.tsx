...
function leaseCreatePath(record: ViewingRecordOut) {
  return `/dashboard/rental/leases?source_viewing_record_id=${record.id}`;
}
      ...
      >
        <Space wrap>
          {readyLeaseCount > 0 ? (
            <Button
              type="primary"
              href="/dashboard/rental/viewings?pending_lease=true&contact_missing=false"
            >
              查看可签约
            </Button>
          ) : null}
          {pendingLeaseCount > 0 ? (
            <Button href="/dashboard/rental/viewings?pending_lease=true">
              查看待签约
            </Button>
          ) : null}
