...
vi.mock('@/components/EntityPreview', () => ({
  HousePreview: ({
    ...
    <span data-preview="house" data-id={id}>
      {children}
    </span>
  ),
}));
...
const houseSummary = (options: {
  id: number;
  roomNumber: string;
  building?: typeof building1;
}) => ({
  id: options.id,
...
describe('Property rental workbench', () => {
  beforeEach(() => {
    ...
    mockListHouses.mockImplementation((params?: Record<string, unknown>) => {
      const blockedHouse = {
        ...houseSummary({ id: 1, roomNumber: '101' }),
        landlord_id: null,
        landlord: null,
        images: [],
        videos: [],
      ...
      if (params?.publish_issue)
        return Promise.resolve({ items: [], total: 1, page: 1, page_size: 1 });
      if (params?.publish_blocked && params?.page_size === 1)
        return Promise.resolve({ items: [], total: 1, page: 1, page_size: 1 });
      if (params?.publish_ready && params?.page_size === 1)
        return Promise.resolve({ items: [], total: 1, page: 1, page_size: 1 });
          ...
          items: [
            {
              id: 4,
              house_id: 1,
              house: houseSummary({ id: 1, roomNumber: '201' }),
              contact_id: null,
              contact: null,
              customer_name: '李客户',
          ...
          items: [
            {
              id: 6,
              house_id: 1,
              house: houseSummary({ id: 1, roomNumber: '202' }),
              customer_name: '王客户',
      ...
      items: [
        {
          id: 5,
          house_id: 1,
          house: houseSummary({ id: 1, roomNumber: '301' }),
          tenant_id: 6,
  ...
  it('builds publish and workflow rows from API items without extra page wrappers', () => {
    ...
    expect(publishRows[0]).toMatchObject({
      key: 'blocked-1',
      stage: 'blocked',
    });
    ...
    expect(workflowRows.map((row) => row.queueKey)).toEqual([
      'contact-missing',
      'converted',
    ]);
  });
  ...
  it('restores publish and workflow filters from URL search params', async () => {
    window.history.pushState(
      {},
      '',
      '/dashboard/rental/workbench/overview?publish=blocked&workflow=contract',
    );
  ...
  it('syncs workbench filters back to URL', async () => {
    ...
    const segmentedGroups = screen.getAllByRole('radiogroup', {
    ...
    fireEvent.click(
      within(workflowGroup).getByText((text) => text.startsWith('待签约 ')),
    );
    await waitFor(() =>
      expect(window.location.search).toBe(
        '?publish=blocked&workflow=converted',
      ),
    );

    fireEvent.click(
      within(publishGroup).getByText((text) => text.startsWith('全部 ')),
    );
    await waitFor(() =>
      expect(window.location.search).toBe('?workflow=converted'),
    );
  });
  ...
  it('does not request publish issue shortcut counts', async () => {
    mockListHouses.mockImplementation((params?: Record<string, unknown>) => {
      ...
      if (params?.publish_issue === 'rent')
        return Promise.resolve({ items: [], total: 8, page: 1, page_size: 1 });
      if (params?.publish_issue === 'cover')
        return Promise.resolve({ items: [], total: 9, page: 1, page_size: 1 });
      if (params?.publish_issue === 'images')
        return Promise.resolve({ items: [], total: 10, page: 1, page_size: 1 });
      if (params?.publish_issue === 'floor_plan')
        return Promise.resolve({ items: [], total: 11, page: 1, page_size: 1 });
      if (params?.publish_blocked && params?.page_size === 1)
        return Promise.resolve({ items: [], total: 77, page: 1, page_size: 1 });
      if (params?.publish_ready && params?.page_size === 1)
        return Promise.resolve({ items: [], total: 66, page: 1, page_size: 1 });
      if (params?.publish_blocked)
        return Promise.resolve({
          items: [],
          total: 77,
    ...
    expect(mockListHouses).not.toHaveBeenCalledWith(
      expect.objectContaining({ publish_issue: 'landlord', page_size: 1 }),
    );
    expect(mockListHouses).not.toHaveBeenCalledWith(
  ...
  it('confirms ready-house publishing from workbench before mutating status', async () => {
    ...
    await waitFor(() =>
      expect(mockPatchHouse).toHaveBeenCalledWith(3, { status: 'listed' }),
    );
  });
});
