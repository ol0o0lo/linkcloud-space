...
describe('Property rental domain list pages', () => {
  ...
  it('shows estate and building rows', async () => {
    ...
    expect(screen.getByRole('link', { name: '1 栋' })).toHaveAttribute(
      'href',
      '/property-rental/buildings/2',
    );
    expect(
      document.querySelector('svg[viewBox="0 0 24 24"]')?.getAttribute('style'),
    ).toBe(
      'display: inline-block; margin-inline-end: 4px; vertical-align: -0.125em;',
    );
  ...
  it('uses a dedicated building avatar in building rows', async () => {
    ...
    ).toHaveAttribute('aria-label', 'bank');
  });
  ...
  it('updates a building with the confirmed map coordinates', async () => {
    ...
    renderPage(<EstatesPage />);
    fireEvent.click(
      (await screen.findAllByRole('button', { name: '编辑' }))[1],
    );
  ...
  it('filters buildings by estate_id from the URL and clears the estate filter', async () => {
    ...
    window.history.pushState(
      {},
      '',
      '/property-rental/estates?view=buildings&estate_id=1',
    );
  ...
  it('keeps the estate filter when opening the pending-location task', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/estates?view=buildings&task=building_location&estate_id=1',
    );
    mockListBuildings.mockResolvedValue({
      items: [
        buildingItem({
  ...
  it('shows associated buildings only while editing an estate that has buildings', async () => {
    ...
    expect(screen.getByRole('link', { name: '查看全部楼栋' })).toHaveAttribute(
      'href',
      '/dashboard/property-rental/estates?view=buildings&estate_id=1',
    );
  });
  ...
  it('shows estate-associated buildings from the estate query when the current keyword does not match', async () => {
    ...
    window.history.pushState(
      {},
      '',
      '/property-rental/estates?keyword=%E4%B8%8D%E5%8C%B9%E9%85%8D',
    );
  ...
  it('links active buildings directly to house registration', async () => {
    ...
    expect(
      ...
      'href',
      '/dashboard/property-rental/houses/new?building_id=2',
    );
  });
  ...
  it('restores estate search state from URL', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/estates?keyword=%E6%97%A7%E6%94%B9&view=buildings&estate_page=2&building_page=3',
    );
  ...
  it('loads every building for estate coverage without inheriting the current keyword or estate filter', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/estates?keyword=%E7%AD%9B%E9%80%89&estate_id=11',
    );
  ...
  it('filters estate governance queue for building address issues from URL', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/estates?task=building_address',
    );
    mockListEstates.mockResolvedValue({
      items: [
        {
  ...
  it('filters estate governance queue for estate address issues from URL', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/estates?task=estate_address',
    );
    mockListEstates.mockResolvedValue({
      items: [
        {
  ...
  it('opens building creation drawer directly from the no-building queue context and clears edit state on close', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/estates?task=no_building&building_create=1',
    );
    mockListEstates.mockResolvedValue({
      items: [
        {
  ...
  it('clears legacy missing-role filters from the URL', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/contacts?task=role_missing_active',
    );
  ...
  it('从 URL 打开联系人编辑抽屉并在关闭时清除 edit 参数', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/contacts?edit=3&role=landlord',
    );
    mockListContacts.mockResolvedValue({
      items: [],
      total: 0,
  ...
  it('restores contact filters from URL search params', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/contacts?role=tenant&keyword=%E7%8E%8B%E7%A7%9F%E5%AE%A2&page=2',
    );
    mockListContacts.mockImplementation((params?: Record<string, unknown>) => {
      if (
        params?.role === 'tenant' &&
  ...
  it('restores contact task filters from URL search params', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/contacts?task=inactive&keyword=%E5%81%9C%E7%94%A8&page=2',
    );
    mockListContacts.mockImplementation((params?: Record<string, unknown>) => {
      if (
        params?.keyword === '停用' &&
  ...
  it('ignores legacy house task params', async () => {
    ...
    expect(
      ...
      'href',
      '/dashboard/property-rental/houses/99?action=edit',
    );
    expect(screen.queryByText(/当前只看/)).not.toBeInTheDocument();
  ...
  it('filters houses by building_id from the URL and preserves unrelated query params when clearing the building filter', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/houses?building_id=11&foo=x',
    );
  ...
  it('preserves unrelated URL params when changing the house status filter', async () => {
    window.history.pushState(
      {},
      '',
      '/dashboard/property-rental/houses?building_id=11&foo=x',
    );
  ...
  it('shows house layout and requested listing facts in the house list', async () => {
    ...
    expect(await screen.findByText('单间 · 1卫')).toBeInTheDocument();
    expect(
      screen.getByText('38.50㎡ · 12层 · 朝南 · 精装'),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByRole('columnheader', { name: '电梯' }),
    ).toBeInTheDocument();
    expect(screen.getAllByText('电梯').length).toBeGreaterThan(1);
    expect(screen.getByText('张房东')).toBeInTheDocument();
    expect(screen.queryByText('张房东 / 13800000000')).not.toBeInTheDocument();
    expect(
      screen.getByRole('columnheader', { name: '媒体' }),
    ).toBeInTheDocument();
  ...
  it('requires confirmation before publishing a house from the list', async () => {
    ...
    fireEvent.click(
      within(publishRow as HTMLElement).getByRole('button', { name: 'more' }),
    );
  ...
  it('uses organization publish rules before opening list publish confirmation', async () => {
    ...
    expect(
      screen.queryByText('确认后房源状态将切换为招租中，继续承接带看。'),
    ).not.toBeInTheDocument();
  ...
  it('uses a house avatar placeholder when a house has no cover image', async () => {
    ...
    expect(
      await screen.findByTestId('house-image-placeholder'),
    ).toBeInTheDocument();
  });
  ...
  it('renders keyword and status filters in the house table toolbar', async () => {
    ...
    expect(
      await screen.findByPlaceholderText('搜索房号 / 项目 / 楼栋 / 房东'),
    ).toBeInTheDocument();
  ...
  it('restores house keyword from URL search params', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/houses?keyword=A-101&status=vacant&page=2',
    );
  ...
  it('syncs house search state back to URL', async () => {
    ...
    fireEvent.change(
      screen.getByPlaceholderText('搜索房号 / 项目 / 楼栋 / 房东'),
      { target: { value: 'A-101' } },
    );
  ...
  it('filters converted viewings from URL and links to lease creation', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/viewings?status=converted',
    );
    mockListViewings.mockResolvedValue({
      items: [
        viewingItem({
    ...
    expect(within(row).getByRole('link', { name: '签约' })).toHaveAttribute(
      'href',
      '/dashboard/property-rental/leases?source_viewing_record_id=4',
    );
  ...
  it('filters pending lease viewings from URL', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/viewings?pending_lease=true',
    );
  ...
  it('restores viewing status and page filters from URL search params', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/viewings?status=viewed&page=2',
    );
    mockListViewings.mockImplementation((params?: Record<string, unknown>) => {
      if (params?.status === 'scheduled') {
        return Promise.resolve({ items: [], total: 0, page: 1, page_size: 1 });
  ...
  it('keeps ready-to-lease overview aligned with the filtered queue', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('查看待补租客').closest('a')).toHaveAttribute(
        'href',
        '/dashboard/property-rental/viewings?pending_lease=true&contact_missing=true',
      );
      expect(screen.getByText('查看待签约').closest('a')).toHaveAttribute(
        'href',
        '/dashboard/property-rental/viewings?pending_lease=true',
      );
    });
  });
  ...
  it('keeps missing-contact overview aligned with the filtered queue', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('查看可签约').closest('a')).toHaveAttribute(
        'href',
        '/dashboard/property-rental/viewings?pending_lease=true&contact_missing=false',
      );
      expect(screen.getByText('查看待签约').closest('a')).toHaveAttribute(
        'href',
        '/dashboard/property-rental/viewings?pending_lease=true',
  ...
  it('opens viewing edit drawer directly from URL query', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/viewings?pending_lease=true&edit=4',
    );
    mockListViewings.mockResolvedValue({
      items: [viewingItem({ status: 'converted', signed_lease_id: null })],
      total: 1,
  ...
  it('clears focused viewing edit context when the drawer closes', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/viewings?pending_lease=true&contact_missing=true&edit=4&task=contact',
    );
    mockListViewings.mockResolvedValue({
      items: [viewingItem({ status: 'converted', signed_lease_id: null })],
      total: 1,
  ...
  it('shows contact business actions for landlord and tenant roles', async () => {
    ...
    expect(
      ...
      'href',
      '/dashboard/property-rental/houses/new?landlord_id=3',
    );
    expect(screen.getByRole('link', { name: '登记带看' })).toHaveAttribute(
      'href',
      '/dashboard/property-rental/viewings?contact_id=6',
    );
  });
  ...
  it('restores lease status and page filters from URL search params', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/leases?status=active&page=2',
    );
    mockListLeases.mockImplementation((params?: Record<string, unknown>) => {
      if (params?.status === 'pending') {
        return Promise.resolve({ items: [], total: 1, page: 1, page_size: 1 });
  ...
  it('shows a task-aware empty state when the contract queue is cleared', async () => {
    ...
    expect(screen.queryByText(/当前只看/)).not.toBeInTheDocument();
    expect(
      await screen.findByText('合同缺失队列已处理完成'),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
    ...
    expect(
      await screen.findByRole('link', { name: '查看全部租约' }),
    ).toHaveAttribute('href', '/dashboard/property-rental/leases');
    expect(
      screen.getByRole('button', { name: '新建租约' }),
    ).toBeInTheDocument();
  });
  ...
  it('filters leases by missing contract task and exposes contract upload field', async () => {
    ...
    await waitFor(() =>
      expect(mockListLeases).toHaveBeenCalledWith(
        expect.objectContaining({ contract_missing: true }),
      ),
    );
    expect(
      screen.queryByRole('button', { name: /待补合同/ }),
    ...
    expect(screen.getByRole('button', { name: '编辑' })).toBeInTheDocument();
    expect(
      screen.queryByRole('link', { name: '补合同' }),
    ).not.toBeInTheDocument();
  ...
  it('surfaces combined contract and status scope when leases are filtered by both', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/leases?task=contract&status=pending',
    );
    mockListLeases.mockImplementation((params?: Record<string, unknown>) => {
  ...
  it('opens lease creation drawer from converted viewing source', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/leases?source_viewing_record_id=4',
    );
    mockListViewings.mockImplementation((params?: Record<string, unknown>) => {
  ...
  it('opens lease edit drawer from edit query param', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/leases?house_id=99&edit=5',
    );
  ...
  it('clears edit query params when closing the lease edit drawer', async () => {
    window.history.pushState(
      {},
      '',
      '/property-rental/leases?house_id=99&task=contract&edit=5',
    );
  ...
  it('keeps contract queue context when opening a lease edit drawer from the contract task', async () => {
    ...
    expect(screen.queryByText(/当前只看/)).not.toBeInTheDocument();
    expect(
      screen.queryByText('当前只看：房源：A-101 / 合同缺失'),
    ).not.toBeInTheDocument();
  ...
  it('routes incomplete source viewings back to the viewing workflow before signing', async () => {
    ...
    expect(screen.getByRole('link', { name: '去补租客' })).toHaveAttribute(
      'href',
      '/dashboard/property-rental/viewings?pending_lease=true&contact_missing=true&edit=4',
    );
    expect(
      screen.queryByText('已带入成交带看，补齐租期和金额后保存。'),
    ).not.toBeInTheDocument();
  ...
  it('shows the next lease step inside the drawer summary', async () => {
    ...
    expect(
      screen.getByText('合同可稍后在编辑租约中补充。'),
    ).toBeInTheDocument();
    expect(screen.getByRole('link', { name: '去待签约带看' })).toHaveAttribute(
      'href',
      '/dashboard/property-rental/viewings?pending_lease=true&contact_missing=false',
    );
    expect(screen.getByRole('link', { name: '去待补租客' })).toHaveAttribute(
      'href',
      '/dashboard/property-rental/viewings?pending_lease=true&contact_missing=true',
    );
  });
