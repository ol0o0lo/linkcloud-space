...
- 技术栈：React、Ant Design Pro、Umi Max、pnpm
...
管理后台与小程序端继续按各自子工程方式安装和运行依赖：

- 开发后台时，优先使用 `just admin_dev`
- 构建后台产物时，使用 `just admin_build`
- 小程序端继续通过 `just miniprogram_dev`、`just miniprogram_build`、`just miniprogram_build_h5` 维护
...
如果某个功能同时涉及这两个入口，应该共享的是后端接口、权限规则和状态定义，而不是直接拷贝或互相引用前端页面实现。
